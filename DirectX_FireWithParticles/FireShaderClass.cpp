...
bool FireShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	WCHAR fx[256] = L"data/fire/fire.fx";
	// Initialize the shader that will be used to draw the triangles.
	result = InitializeShader(device, hwnd, fx);
	if (!result)
	{
		return false;
	}
...
void FireShaderClass::Render(ID3D11DeviceContext* device, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
...
bool FireShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* filename)
{
	HRESULT result;
	...
	/*result = D3DCompileFromFile(filename, NULL, NULL, "FireVertexShader", "fx_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
								&vertexShaderBuffer, &errorMessage);
	result = D3DCompileFromFile(filename, NULL, NULL, NULL, "fx_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
							    &vertexShaderBuffer, &errorMessage);

	result = D3DX10CreateEffectFromFile(filename, NULL, NULL, "fx_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, 
										device, NULL, NULL, &m_effect, &errorMessage, NULL);*/
	result = D3DCompileFromFile(filename, NULL, NULL, NULL, "fx_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
		&effectBlob, &errorMessage);
	//D3DX11CompileFromFile("Effect1.fx", 0, 0, 0, "fx_5_0", 0, 0, 0, &effectBlob, &errorsBlob, 0);
	
	//result = D3DX11CreateEffectFromFile(filename, 0, device, &m_effect);
	if (FAILED(result))
	{
	...
	result = D3DX11CreateEffectFromMemory(effectBlob->GetBufferPointer(), effectBlob->GetBufferSize(), 0, device, &m_effect);
...
void FireShaderClass::OutputShaderErrorMessage(ID3DBlob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
void FireShaderClass::SetShaderParameters(XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* fireTexture, ID3D11ShaderResourceView* noiseTexture,
	ID3D11ShaderResourceView* alphaTexture, float frameTime, XMFLOAT3 scrollSpeeds,
	XMFLOAT3 scales, XMFLOAT2 distortion1, XMFLOAT2 distortion2,
	XMFLOAT2 distortion3, float distortionScale, float distortionBias)
{
...
void FireShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	{
		m_technique->GetPassByIndex(i)->Apply(0, deviceContext);
